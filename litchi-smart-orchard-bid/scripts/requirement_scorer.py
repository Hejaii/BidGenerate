...
class QianwenScorer:
    def __init__(self, api_key: str):
        ...
        # 验证API密钥格式
        print(f"正在验证API密钥: {self.api_key[:10]}...")
...
class ProjectFileAnalyzer:
    ...
    def _scan_project_files(self):
        """扫描项目文件"""
        print(f"正在扫描项目文件: {self.project_root}")
        ...
        for file_path in self.project_root.rglob("*"):
            if file_path.is_file() and file_path.suffix not in ['.md', '.py', '.sh']:
                try:
                    # 对于非文本文件，只记录基本信息
                    file_info = {
                        "name": file_path.name,
        ...
        print(f"扫描完成，共找到 {len(self.project_files)} 个文件")
    ...
    def find_relevant_files(self, requirement: Dict) -> List[Dict]:
        ...
        return relevant_files[:5]  # 返回前5个最相关的文件
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        # 技术相关关键词
        tech_keywords = ["技术", "系统", "软件", "硬件", "设备", "功能", "性能", "架构", "集成", "测试"]
        # 商务相关关键词
        business_keywords = ["资质", "证书", "业绩", "经验", "团队", "管理", "质量", "安全", "成本", "进度"]
...
class RequirementScorer:
    def __init__(self, api_key: str, project_root: str):
        self.qianwen_scorer = QianwenScorer(api_key)
        self.project_analyzer = ProjectFileAnalyzer(project_root)
    ...
    def load_requirements(self, requirements_file: str) -> List[Dict]:
        ...
        try:
            ...
            for page_num, summary, score, text in requirement_blocks:
                # 判断类别
                category = "技术要求" if any(tech in text for tech in ["技术", "系统", "软件", "硬件"]) else "商务要求"
    ...
    def score_all_requirements(self, requirements: List[Dict]) -> List[Dict]:
        ...
        for i, requirement in enumerate(requirements):
            ...
            print(f"要求: {requirement.get('requirement_summary', '')[:50]}...")
            
            # 找到相关文件
            relevant_files = self.project_analyzer.find_relevant_files(requirement)
            print(f"找到 {len(relevant_files)} 个相关文件")
    ...
    def save_results(self, output_dir: str):
        ...
        # 保存详细结果
        detailed_path = os.path.join(output_dir, "detailed_scoring_results.json")
        with open(detailed_path, 'w', encoding='utf-8') as f:
            json.dump(self.scoring_results, f, ensure_ascii=False, indent=2)
        print(f"详细评分结果已保存到: {detailed_path}")
        
        # 保存评分报告
        report_path = os.path.join(output_dir, "scoring_report.txt")
        report = self.generate_scoring_report()
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(report)
        print(f"评分报告已保存到: {report_path}")
...
def main():
    ...
    if not os.path.exists(requirements_file):
        print(f"错误: 找不到要求文件 {requirements_file}")
        return
    ...
    # 创建评分器
    scorer = RequirementScorer(api_key, project_root)
    
    print("=== 标文要求评分程序 ===")
    print(f"要求文件: {requirements_file}")
    print(f"项目目录: {project_root}")
    print(f"输出目录: {output_dir}")
    
    # 加载要求
    requirements = scorer.load_requirements(requirements_file)
    if not requirements:
